...
public class NameGenData {
    ...
    private static void parseTrainingNamesForAttribute(NameAttribute attribute){
        ...
        WeightedNameData firsts = new WeightedNameData(new NameData(fbegs, fmids, fends, fmean, Math.sqrt(fmeanTotal / i)));
        WeightedNameData lasts = new WeightedNameData(new NameData(lbegs, lmids, lends, lmean, Math.sqrt(lmeanTotal / i)));
    ...
    public static String makeName(NameAttribute attribute){
        ...
        if (attribute != null) {
            ...
            return makeName(attributeNameData.get(attribute).first) + " " + makeName(attributeNameData.get(attribute).last);
        } else {
            return makeName(first) + " " + makeName(last);
        }
    }
